(function (world) {
    ...
    world.initialize = function () {
        ...
        if (initDevice !== "" && initDevice != nullDevice) {
    ...
    world.set_selected_device = function (name, force) {
        ...
        if (device === null) {
            if (name != "" && name != nullDevice) {
                error('unable to find device with name:',name,'\n')
            }
            theDevice = null;
        ...
        if (initialized) {
            post('clear storage\n');
            world.outlet(storageOutput,"clear")
        }
        theDevice = null;
        post('create device\n');
        ...
        if (initialized) {
            ...
            world.outlet(storageOutput,"store",1);
        }
    ...
    world.setvalueof = function (state) {
        // post('setvalueof','side:',recallSide,'\n');
        if (theDevice === null) {
            return;
        }
