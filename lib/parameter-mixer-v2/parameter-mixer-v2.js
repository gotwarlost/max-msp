(function (world) {
    ...
    const devicesOutlet = 0;
    const recallSideOutlet = 1;
    const dumpOutlet = 2;
    ...
    var recallSide = 0; // the side to which presets are recalled (0 = left, 1 = right)
    var forcedRecallSide = -1; // programmatic override for recall side
    ...
    world.initialize = function () {
        ...
        const items = [devicesOutlet, "device_list"];
        for (var i=0; i<list.length; i++) {
            items.push(list[i]);
        }
        world.outlet.apply(world, items);
        if (initDevice != "") {
            world.set_selected_device(initDevice);
            initDevice = "";
        }
    }

    world.set_selected_device = function (name) {
        ...
        world.outlet(devicesOutlet,"selected_device", name);
    }
    ...
    world.setvalueof = function (state) {
        ...
        const side = forcedRecallSide !== -1 ? forcedRecallSide : recallSide;
        // post('state:', state, typeof state, 'right:', right,typeof right,'\n');
        if (!(typeof state === 'string')) {
            error('state was not a string:', typeof state,'\n');
            return;
        }
        const values = JSON.parse(state);
        theDevice.params.setValues(values,side);
    }
    ...
    world.set_forced_recall_side = function (side) {
        forcedRecallSide = side;
    }

    world.get_recall_side = function () {
        const side = forcedRecallSide !== -1 ? forcedRecallSide : recallSide;
        world.outlet(recallSideOutlet, side);
    }
